...
export class FoodServiceService {
  ...
  getFoodById(foodId:number): IFood{
      return this.allFood.find(food=>food.id == foodId) ?? this.dummYFood
  }

  getTagByFoodId(foodId: number): string[]{
    let food = this.allFood.find(food=>food.id == foodId);
    if(food){
      return food['tags']
    }
    return []
